 ...
 * @version
 */
@Service("infoqueryFileBatchService")
public class FileBatchService {
	...
	public List<Map<String, Object>> listService(int pageNum, int pageSize, PageData pd) throws Exception{
		...
		Object object = dao.findForList("com.zyx.mapper.infoquery.FileBatchMapper.list", pd);
	...
	public String saveService(PageData pd) throws Exception {
		int count = (int) dao.findForObject("com.zyx.mapper.infoquery.FileBatchMapper.isExists", pd);
		if(count > 0){
			return "主键ID已存在!";
		}else{
			dao.save("com.zyx.mapper.infoquery.FileBatchMapper.save", pd);
	...
	public Map<String, Object> editService(PageData pd) throws Exception {
		Object object = dao.findForObject("com.zyx.mapper.infoquery.FileBatchMapper.listById", pd);
	...
	public String updateService(PageData pd) throws Exception {
		int count = (int) dao.findForObject("com.zyx.mapper.infoquery.FileBatchMapper.isExists", pd);
		if(count > 0){
			dao.update("com.zyx.mapper.infoquery.FileBatchMapper.update", pd);
			return Constants.SUCCESS;
		}else{
			return "记录已被其他用户删除!";		
		}
	...
	public String delService(PageData pd) throws Exception {
		dao.delete("com.zyx.mapper.infoquery.FileBatchMapper.delete", pd);
		return Constants.SUCCESS;
	}
