 ...
 * @version
 */
@Service("infoqueryErrMsgService")
public class ErrMsgService {
	...
	public List<Map<String, Object>> listService(int pageNum, int pageSize, PageData pd) throws Exception{
		...
		Object object = dao.findForList("com.zyx.mapper.infoquery.ErrMsgMapper.list", pd);
	...
	public String saveService(PageData pd) throws Exception {
		int count = (int) dao.findForObject("com.zyx.mapper.infoquery.ErrMsgMapper.isExists", pd);
		if(count > 0){
			return "主键ID已存在!";
		}else{
			dao.save("com.zyx.mapper.infoquery.ErrMsgMapper.save", pd);
	...
	public Map<String, Object> editService(PageData pd) throws Exception {
		Object object = dao.findForObject("com.zyx.mapper.infoquery.ErrMsgMapper.listById", pd);
	...
	public String updateService(PageData pd) throws Exception {
		int count = (int) dao.findForObject("com.zyx.mapper.infoquery.ErrMsgMapper.isExists", pd);
		if(count > 0){
			dao.update("com.zyx.mapper.infoquery.ErrMsgMapper.update", pd);
			return Constants.SUCCESS;
		}else{
			return "记录已被其他用户删除!";		
		}
	...
	public String delService(PageData pd) throws Exception {
		dao.delete("com.zyx.mapper.infoquery.ErrMsgMapper.delete", pd);
		return Constants.SUCCESS;
	}
